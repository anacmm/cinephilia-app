...
class MovieControllerTest {
  ...
  @MockBean private MovieApi movieApi;
  ...
  void getPopularMovies_ShouldReturnMoviesList() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/movies/popular").contentType(MediaType.APPLICATION_JSON))
        .andDo(print())
  ...
  void getTopRatedMovies_ShouldReturnMoviesList() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/movies/top-rated").contentType(MediaType.APPLICATION_JSON))
        .andDo(print())
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
        .andExpect(jsonPath("$[0].id").value(1))
  ...
  void getMovieDetails_ShouldReturnMovie() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/movies/{id}", movieId).contentType(MediaType.APPLICATION_JSON))
        .andDo(print())
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
